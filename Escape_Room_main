...
key = pygame.image.load("Images/key.png").convert_alpha()
key_icon = pygame.transform.scale(key, (70, 25))
pencil = pygame.image.load("Images/pencil.png").convert_alpha()
pencil_icon = pygame.transform.scale(pencil, (70, 25))
...
restart_button = pygame.image.load("Images/restart_button.png").convert_alpha()
...
door_rect = door.get_rect(topleft=(976, 180))
...
board_rect = board.get_rect(topleft=(872, 232))
...
pencil_rect = pencil.get_rect(topleft=(540, 130))
eraser_rect = eraser.get_rect(topleft=(690, 310))
character_rect = character.get_rect(topleft=(480, 280))
blindfold_rect = blindfold.get_rect(topleft=(504, 301))
restart_button_rect = restart_button.get_rect(topleft=(513, 306))
...
C2_front_rect = C2_front.get_rect(topleft=(850, 410))  
...
def reset_game():
    # This function resets ALL game state back to the start so the player can replay
    global current_view, inventory_items, key_visible, pencil_visible, eraser_visible
    global blindfold_visible, drawer1_open, drawer2_open, drawer3_open, drawer_locked
    global selected_item, character, A1_flipped, A2_flipped, B1_flipped, B2_flipped
    global C1_flipped, C2_flipped, card1_name, card1_value, matches, mirror_unlocked
    global mirror_black, mirror_black_closeup, show_door_message, show_mirror_message
    global show_mirror2_message, show_drawer_message, show_warning_message, mc_message
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # BOARD VIEW
                ...
                # If the player clicks the restart button, reset everything and go back to room1
